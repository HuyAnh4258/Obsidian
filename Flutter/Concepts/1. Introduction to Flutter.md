...
| **Single Codebase**  | Viết một lần → Chạy đa nền tảng                                                                                                                       |
| **Performance**      | Flutter sử dụng ngôn ngữ **Dart** để biên dịch **AOT (Ahead-of-Time)** trực tiếp thành mã máy mã gốc (ARM hoặc x86) để đạt tốc độ gần bằng **native** |
|                      |                                                                                                                                                       |
...
```
┌─────────────────────────────────────────┐
│           Your Flutter App              │  ← Dart code bạn viết
├─────────────────────────────────────────┤
│        Tầng 1: Flutter Framework        │  ← Widgets, Material, Cupertino
│     - Widgets (Stateless, Stateful)     │
|     - Hệ thống Render và Layout         |
|     - Các thư viện nền tảng             |
├─────────────────────────────────────────┤
│       Tầng 2: Flutter Engine (C++)      │  ← Skia/Impeller, Dart VM
|   - Được viết bằng C++, chạy bằng Skia  |
|Xử lý render, accessibility, text, layout|
|  - Tăng tốc CPU đảm bảo tính chính xác  |
├─────────────────────────────────────────┤
│       Tầng 3: Platform Embedder         │  ← Android/iOS/Web/Desktop
|  - Tích hợp với từng Platform cụ thể    |
|Xử lý nhập vào, lifecycle, quản lý window|
|- Các embedder tuỳ chỉnh cho Raspberry Pi|
└─────────────────────────────────────────┘
```
